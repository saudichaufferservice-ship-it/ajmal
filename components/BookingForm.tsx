...
import { Calendar, Clock, Users, MapPin, Briefcase, MessageCircle, Navigation, Car } from 'lucide-react';
...
export default function BookingForm({ dark = false, title = "General Inquiry" }: BookingFormProps) {
  const [formData, setFormData] = useState({
    pickup: '', drop: '', date: '', time: '', passengers: '5', luggage: '1', vehicle: 'GMC Yukon XL'
  });
  ...
  const vehiclesList = [
    "GMC Yukon XL", "Mercedes S-Class", "Cadillac Escalade", "BMW 7 Series", 
    "Genesis G80", "Ford Taurus", "Mercedes Vito", "Mercedes Sprinter", "Luxurious Bus"
  ];
    ...
    <form onSubmit={handleSubmit}>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-5">
        ...
        <div>
          ...
          <div className="relative">
            <Car className={iconClass} size={16} />
            <select name="vehicle" value={formData.vehicle} className={`${inputClass} appearance-none cursor-pointer`} onChange={handleChange}>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <select name="passengers" value={formData.passengers} className={`${inputClass} appearance-none cursor-pointer`} onChange={handleChange}>
              {[1,2,3,4,5,6,7,10,14,30].map(n => <option key={n} value={n}>{n} {n === 1 ? 'passenger' : 'passengers'}</option>)}
            </select>
          </div>
        </div>
        <div>
          ...
          <div className="relative">
            ...
            <select name="luggage" value={formData.luggage} className={`${inputClass} appearance-none cursor-pointer`} onChange={handleChange}>
              {[1,2,3,4,5,6,7,8,15,40].map(n => <option key={n} value={n}>{n} {n === 1 ? 'bag' : 'bags'}</option>)}
            </select>
          </div>
        </div>
      </div>
